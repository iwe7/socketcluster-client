...
function SCClientSocket(socketOptions) {
  ...
  // TODO 2: Change all let to let or const.
...
SCClientSocket.prototype._handleBrowserUnload = async function () {
  ...
  (async () => {
    let asyncIterator = transport.listener('connecting').createAsyncIterator();
    while (true) {
...
SCClientSocket.prototype._changeToUnauthenticatedStateAndClearTokens = function () {
  if (this.authState !== this.UNAUTHENTICATED) {
    ...
    let oldSignedToken = this.signedAuthToken;
    this.authState = this.UNAUTHENTICATED;
    this.signedAuthToken = null;
    this.authToken = null;
